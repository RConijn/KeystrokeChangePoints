...
estimate_cp <- function(dataset, sessionid, vars, maxcp = 10, timeinterval = 1,
  ...
  if (is.na(out$trend$ncp)){
    ...
    output2 <- data.frame(
      ...
      cp_ci_lower = out$trend$cpCI[,1],
      cp_ci_upper = out$trend$cpCI[,2],
      cp_loc = out$trend$cp,
...
write.csv(beast_1, paste0("app/models4/rbeast_time", timeinterval, trimyes, "_", 
