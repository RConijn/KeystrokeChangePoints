...
# plot accuracies  
beast_pick_sum %>%
  pivot_longer(cols = c("perc_correct", "perc_almost_correct", 
                        "perc_almost60_correct")) %>%
  mutate(Accuracy = ordered(as.character(name),
                            levels = c("perc_correct", "perc_almost_correct", 
                                       "perc_almost60_correct"),
                            labels = c("Correct", "Within 10sec", "Within 60sec"))) %>%
  filter(!is.na(m_seconds_off)) %>%
  ggplot() +
  geom_bar(aes(y = value, x = session, color = Accuracy, fill = Accuracy), 
           stat = "identity", position ="identity", alpha = 0.5) +
  theme_bw() +
  theme(text = element_text(size=14)) +
  scale_y_continuous(labels = scales::percent) +
  ylab("Percentage of change points correct") +
  xlab("Session") 
